...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let kafka_servers =
        std::env::var("KAFKA_BOOTSTRAP_SERVERS").unwrap_or_else(|_| "localhost:9093".to_string());
    let message_timeout =
        std::env::var("KAFKA_MESSAGE_TIMEOUT_MS").unwrap_or_else(|_| "5000".to_string());
    let delivery_timeout =
        std::env::var("KAFKA_DELIVERY_TIMEOUT_MS").unwrap_or_else(|_| "5000".to_string());
    let request_timeout =
        std::env::var("KAFKA_REQUEST_TIMEOUT_MS").unwrap_or_else(|_| "5000".to_string());
    let socket_timeout =
        ...
        .fetch_metadata(None, Duration::from_secs(5))
    {
        ...
        Err(e) => {
            log::warn!(
                "Could not fetch Kafka metadata (connection may be unstable): {}",
                e
            );
                ...
                web::post().to(move |req: HttpRequest, body: web::Bytes| {
                    ...
                    async move {
                        ...
                        match producer.send(record, Duration::from_secs(0)).await {
                            ...
                            Err((e, _)) => {
                                log::error!(
                                    "Failed to send message to Kafka topic {} partition {}: {:?}",
                                    topic,
                                    partition,
                                    e
                                );
